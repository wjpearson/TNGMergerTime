...
test_ds = prepare_dataset(list_test_ds, BATCH_SIZE, 200, True)
...
table.write('test_cnn.fits', overwrite=True)


mse = np.mean(np.square(times-latent[:,0]))
print(mse)
print()
print(np.mean(np.abs(times-latent[:,0])))
print(np.median(np.abs(times-latent[:,0])))
print()
print(1500*np.mean(np.abs(times-latent[:,0])))
print(1500*np.median(np.abs(times-latent[:,0])))
