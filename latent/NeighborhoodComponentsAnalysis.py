...
if save:
    embedding = NCA(n_components=2)
    trans = embedding.fit_transform(latents, ttimes)
    with open('../models/latent/NCA_embedding.pkl', "wb") as f:
        pickle.dump(embedding, f)
        f.close()
else:
    with open('../models/latent/NCA_embedding.pkl', "rb") as f:
...
ax[0].text(-200, 650, '(a)', fontsize=16)
...
ax[1].text(-200, 650, '(b)', fontsize=16)
...
if save:
    plt.savefig('latent-NCA.png', bbox_inches='tight')
plt.show()
...
if save:
    ...
    table.write('valid_NCA.fits', overwrite=True)
