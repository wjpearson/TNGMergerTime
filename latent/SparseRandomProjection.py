...
if save:
    embedding = SRP(n_components=2)
    trans = embedding.fit_transform(latents)
    with open('../models/latent/SRP_embedding.pkl', "wb") as f:
        pickle.dump(embedding, f)
        f.close()
else:
    with open('../models/latent/SRP_embedding.pkl', "rb") as f:
...
ax[0].text(-4.6, 4.0, '(a)', fontsize=16)
...
ax[1].text(-4.6, 4.0, '(b)', fontsize=16)
...
if save:
    plt.savefig('latent-SRP.png', bbox_inches='tight')
plt.show()
...
if save:
    ...
    table.write('valid_SRP.fits', overwrite=True)
