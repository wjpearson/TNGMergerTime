...
if save:
    embedding = TSVD(n_components=2)
    trans = embedding.fit_transform(latents)
    with open('../models/latent/TSVD_embedding.pkl', "wb") as f:
        pickle.dump(embedding, f)
        f.close()
else:
    with open('../models/latent/TSVD_embedding.pkl', "rb") as f:
...
ax[0].text(3.03, 2.3, '(a)', fontsize=16)
...
ax[1].text(3.03, 2.3, '(b)', fontsize=16)
...
if save:
    plt.savefig('latent-TSVD.png', bbox_inches='tight')
plt.show()
...
def mse(y_true, y_pred):
    loss = np.nanmean(np.square(y_true - y_pred))
    return loss
...
if save:
    ...
    table.write('valid_TSVD.fits', overwrite=True)
